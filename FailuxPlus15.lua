...
--------------------------------------------------------------------------------
-- HỆ THỐNG ĐỢI 8 GIÂY & CHỮ CHROME "meomeostaylike" (Giữ nguyên từ bản cũ)
--------------------------------------------------------------------------------
local IntroGui = Instance.new("ScreenGui")
IntroGui.Name = "MeomeoIntroGui"
IntroGui.ResetOnSpawn = false
...
local TextLabel = Instance.new("TextLabel")
TextLabel.Size = UDim2.new(0, 400, 0, 50)
TextLabel.Position = UDim2.new(0.5, -200, 0, 30)
TextLabel.BackgroundTransparency = 1
TextLabel.Text = "meomeostaylike"
...
--------------------------------------------------------------------------------
-- Cấu hình UI và Trạng thái mới
--------------------------------------------------------------------------------
local MENU_BG_COLOR = Color3.fromRGB(15, 15, 15)
local MENU_BORDER_COLOR = Color3.fromRGB(138, 43, 226)
local TEXT_COLOR = Color3.fromRGB(240, 240, 240)
local BUTTON_COLOR = Color3.fromRGB(30, 30, 30)
local ACTIVE_COLOR = Color3.fromRGB(45, 20, 20)

-- Biến lưu trạng thái tính năng mới
local barrierCenter: Vector3? = nil
local barrierVisual: Part? = nil
local targetPlayer: Player? = nil
local targetVisual: Part? = nil

local isBarrierActive = false
local isGhimActive = false
local isAntiKetActive = false
local clickToGhimMode = false

local trackConnection: RBXScriptConnection? = nil
local touchConnection: RBXScriptConnection? = nil
local antiKetConnection: RBXScriptConnection? = nil
...
-- NÚT BẬT TẮT "m15" Ở CHÍNH GIỮA TRÊN CÙNG MÀN HÌNH
local ToggleButton = Instance.new("TextButton")
ToggleButton.Name = "m15_Toggle"
ToggleButton.Size = UDim2.new(0, 50, 0, 50)
ToggleButton.Position = UDim2.new(0.5, -25, 0, 10)
ToggleButton.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
ToggleButton.Text = "m15"
...
local MainMenu = Instance.new("Frame")
MainMenu.Name = "MainMenu"
MainMenu.Size = UDim2.new(0, 420, 0, 150)
MainMenu.Position = UDim2.new(0.5, -210, 0.4, -75)
MainMenu.BackgroundColor3 = MENU_BG_COLOR
MainMenu.BorderSizePixel = 0
MainMenu.Active = true
...
-- Xử lý bật tắt Menu bằng nút m15
ToggleButton.MouseButton1Click:Connect(function()
	MainMenu.Visible = not MainMenu.Visible
end)

-- Khung trên chứa Nút Số 4 (Kéo dài gần bằng Menu)
local TopButtonContainer = Instance.new("Frame")
TopButtonContainer.Name = "TopButtonContainer"
TopButtonContainer.Size = UDim2.new(1, -20, 0, 35)
TopButtonContainer.Position = UDim2.new(0, 10, 0, 10)
...
-- Khung dưới chứa Hàng 3 nút nằm ngang
local ButtonContainer = Instance.new("Frame")
ButtonContainer.Name = "ButtonContainer"
...
--------------------------------------------------------------------------------
-- Hàm tạo Vòng tròn Visual Chức năng mới
--------------------------------------------------------------------------------
local function createVisualRing(position: Vector3, size: number, color: Color3): Part
	local part = Instance.new("Part")
	part.Shape = Enum.PartType.Cylinder
	part.Size = Vector3.new(0.2, size, size) 
	part.Color = color
	part.Material = Enum.Material.Neon 
...
--------------------------------------------------------------------------------
-- Khởi tạo các nút chức năng đúng yêu cầu form dáng
--------------------------------------------------------------------------------

-- NÚT 4: Dài bằng menu nằm trên riêng biệt
local btn4 = Instance.new("TextButton")
btn4.Name = "Btn4"
btn4.Size = UDim2.new(1, 0, 1, 0)
btn4.BackgroundColor3 = BUTTON_COLOR
btn4.Text = "Chống Kẹt Tường/Sàn: TẮT"
btn4.TextColor3 = TEXT_COLOR
btn4.TextSize = 13
btn4.Font = Enum.Font.SourceSansBold
btn4.Parent = TopButtonContainer
...
local btn4Stroke = Instance.new("UIStroke")
btn4Stroke.Color = MENU_BORDER_COLOR
btn4Stroke.Thickness = 1.5
btn4Stroke.Parent = btn4

-- Hàm tạo nhanh 3 nút hàng ngang phía dưới
local function createBottomButton(name: string, text: string, order: number)
	local button = Instance.new("TextButton")
	button.Name = name
	...
	stroke.Color = MENU_BORDER_COLOR
	stroke.Thickness = 1
...
local btn1 = createBottomButton("Btn1", "Tạo Kết Giới", 1)
local btn2 = createBottomButton("Btn2", "Ghim Người Chơi", 2)
local btn3 = createBottomButton("Btn3", "Xóa Tọa Độ", 3)

--------------------------------------------------------------------------------
-- LOGIC TÍNH NĂNG CHÍNH
--------------------------------------------------------------------------------

-- Dọn dẹp an toàn hệ thống tọa độ
local function clearAllData()
	if barrierVisual then barrierVisual:Destroy() barrierVisual = nil end
	if targetVisual then targetVisual:Destroy() targetVisual = nil end
	...
	isGhimActive = false
	clickToGhimMode = false
	
	btn1.Text = "Tạo Kết Giới"
	btn1.BackgroundColor3 = BUTTON_COLOR
	btn2.Text = "Ghim Người Chơi"
	btn2.BackgroundColor3 = BUTTON_COLOR
end

-- TÍNH NĂNG 1: Tạo Kết Giới (Vòng tròn siêu to x=100, z=100)
btn1.MouseButton1Click:Connect(function()
	local char = LocalPlayer.Character
	local hrp = char and char:FindFirstChild("HumanoidRootPart") :: Part?
	if not hrp then return end
	
	isBarrierActive = not isBarrierActive
	if isBarrierActive then
		barrierCenter = hrp.Position - Vector3.new(0, 2.8, 0)
		barrierVisual = createVisualRing(barrierCenter, 100, Color3.fromRGB(138, 43, 226))
		btn1.Text = "Kết Giới: ĐANG BẬT"
		btn1.BackgroundColor3 = ACTIVE_COLOR
	else
		if barrierVisual then barrierVisual:Destroy() barrierVisual = nil end
		barrierCenter = nil
		btn1.Text = "Tạo Kết Giới"
...
-- TÍNH NĂNG 2: Click Ghim người chơi (Vòng tròn size 40 bám theo)
btn2.MouseButton1Click:Connect(function()
	if not isBarrierActive then
		btn2.Text = "Hãy Bật Kết Giới Trước!"
		task.wait(1.5)
		btn2.Text = "Ghim Người Chơi"
		return
	end

	clickToGhimMode = not clickToGhimMode
	if clickToGhimMode then
		btn2.Text = "Click Vào Ai Đó..."
		btn2.BackgroundColor3 = ACTIVE_COLOR
	else
		btn2.Text = "Ghim Người Chơi"
		btn2.BackgroundColor3 = BUTTON_COLOR
	end
...
Mouse.Button1Down:Connect(function()
	if not clickToGhimMode or not isBarrierActive then return end
	local target = Mouse.Target
	if not target then return end
	
	local model = target:FindFirstAncestorOfClass("Model")
	if model and model:FindFirstChild("HumanoidRootPart") then
		local clickedPlr = Players:GetPlayerFromCharacter(model)
		if clickedPlr and clickedPlr ~= LocalPlayer then
			targetPlayer = clickedPlr
			clickToGhimMode = false
			isGhimActive = true
			btn2.Text = "Đang Ghim: " .. clickedPlr.Name
			btn2.BackgroundColor3 = ACTIVE_COLOR
			
			if targetVisual then targetVisual:Destroy() end
			if trackConnection then trackConnection:Disconnect() end
			
			local targetHrp = model.HumanoidRootPart :: Part?
			if targetHrp then
				targetVisual = createVisualRing(targetHrp.Position - Vector3.new(0, 2.8, 0), 40, Color3.fromRGB(0, 255, 255))
			end
			
			trackConnection = RunService.PostSimulation:Connect(function()
				if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") and targetVisual then
					local tHrp = targetPlayer.Character.HumanoidRootPart :: Part?
					if tHrp then
						local pos = tHrp.Position - Vector3.new(0, 2.8, 0)
						targetVisual.CFrame = CFrame.new(pos) * CFrame.Angles(0, 0, math.rad(90))
					end
				else
					if targetVisual then targetVisual:Destroy() targetVisual = nil end
					if trackConnection then trackConnection:Disconnect() trackConnection = nil end
...
-- CHỨC NĂNG 3: Chạm vòng ghim -> Tele đến vị trí xa nhất đối với mục tiêu nhưng nằm trong kết giới an toàn
touchConnection = RunService.Heartbeat:Connect(function()
	if not isBarrierActive or not isGhimActive or not barrierCenter or not targetPlayer or not targetPlayer.Character then return end
	
	local char = LocalPlayer.Character
	local hrp = char and char:FindFirstChild("HumanoidRootPart") :: Part?
	local targetHrp = targetPlayer.Character:FindFirstChild("HumanoidRootPart") :: Part?
	if not hrp or not targetHrp then return end
	
	local distToTarget = (hrp.Position - targetHrp.Position).Magnitude
	
	if distToTarget <= 20 then
		local direction = (barrierCenter - targetHrp.Position).Unit
		if direction.Magnitude == 0 or direction ~= direction then 
			direction = Vector3.new(1, 0, 0) 
		end
		
		local maxSafePosition = barrierCenter + (direction * 48)
		hrp.CFrame = CFrame.new(maxSafePosition.X, hrp.Position.Y, maxSafePosition.Z)
	end
end)

-- CHỨC NĂNG 4: Tự Động Kiểm Tra Và Chống Kẹt Khối Block/Sàn/Tường
btn4.MouseButton1Click:Connect(function()
	isAntiKetActive = not isAntiKetActive
	if isAntiKetActive then
		btn4.Text = "Chống Kẹt Tường/Sàn: ĐANG BẬT"
		btn4.BackgroundColor3 = ACTIVE_COLOR
		
		antiKetConnection = RunService.Heartbeat:Connect(function()
			if not isBarrierActive or not barrierCenter then return end
			local char = LocalPlayer.Character
			local hrp = char and char:FindFirstChild("HumanoidRootPart") :: Part?
			...
			overlapParams.FilterDescendantsInstances = {char, barrierVisual, targetVisual}
			
			local partsInside = workspace:GetPartBoundsInBox(hrp.CFrame, Vector3.new(2.2, 3, 2.2), overlapParams)
			
			local isStuck = false
			for _, part in ipairs(partsInside) do
				if part.CanCollide and not part.Locked then
					isStuck = true
					break
				end
			end
			...
			if isStuck then
				...
				local randomRadius = math.random(5, 45)
				local offsetX = math.cos(randomAngle) * randomRadius
				local offsetZ = math.sin(randomAngle) * randomRadius
				
				local escapePos = barrierCenter + Vector3.new(offsetX, hrp.Position.Y - barrierCenter.Y, offsetZ)
		...
		btn4.Text = "Chống Kẹt Tường/Sàn: TẮT"
		btn4.BackgroundColor3 = BUTTON_COLOR
		if antiKetConnection then antiKetConnection:Disconnect() antiKetConnection = nil end
	end
end)
...
btn3.MouseButton1Click:Connect(function()
	clearAllData()
	btn3.Text = "Đã Xóa Sạch!"
	task.wait(1)
	btn3.Text = "Xóa Tọa Độ"
end)
