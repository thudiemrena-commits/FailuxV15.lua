...
-- 2. TẠO KHUNG CHỨA (ĐẢM BẢO TÂM LUÔN GIỮA)
...
-- Dùng UIListLayout để tự động căn giữa (không sợ lệch)
...
createPart("(")
local plusSign = createPart("+") -- Đây là tâm aim
createPart(")")

-- 3. FOV TÀNG HÌNH (1/2 MÀN HÌNH)
...
-- 4. TẠO UI MENU system-meomeohub
...
local function createBtn(text, pos)
    ...
    btn.Text = text
    btn.Size = UDim2.new(0.44, 0, 0.45, 0)
    btn.Position = pos
...
local btnFOV = createBtn("TANK: OFF", UDim2.new(0.04, 0, 0.45, 0))
local btnAim = createBtn("AIM: OFF", UDim2.new(0.52, 0, 0.45, 0))
