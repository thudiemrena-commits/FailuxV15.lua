...
--------------------------------------------------------------------------------
-- HỆ THỐNG ĐỢI 8 GIÂY & CHỮ CHROME "meomeostaylike"
--------------------------------------------------------------------------------
local IntroGui = Instance.new("ScreenGui")
IntroGui.Name = "MeomeoIntroGui"
IntroGui.ResetOnSpawn = false
...
DragLabel.Size = UDim2.new(1, 0, 1, 0)
DragLabel.BackgroundTransparency = 1
DragLabel.Text = "=== GIỮ ĐỂ KÉO MENU (MOBILE) ==="
DragLabel.TextColor3 = Color3.fromRGB(100, 100, 100)
DragLabel.TextSize = 10
DragLabel.Font = Enum.Font.SourceSansBold
DragLabel.Parent = DragBar
...
--------------------------------------------------------------------------------
-- Tính năng kéo thả Menu (Drag System di động)
--------------------------------------------------------------------------------
local dragging = false
local dragInput: InputObject? = nil
local dragStart: Vector3 = Vector3.new()
...
--------------------------------------------------------------------------------
-- LOGIC TÍNH NĂNG CHÍNH (TỐI ƯU MOBILE CHẠM ĐỂ GHIM)
--------------------------------------------------------------------------------
...
btn2.MouseButton1Click:Connect(function()
	...
	scanToGhimMode = not scanToGhimMode
	if scanToGhimMode then
		btn2.Text = "Đang Tìm Mục Tiêu..."
		btn2.BackgroundColor3 = ACTIVE_COLOR
		
		-- Quét người chơi ở gần nhất để ghim tự động trên Mobile
		scanConnection = RunService.Heartbeat:Connect(function()
			if not scanToGhimMode then return end
			local myChar = LocalPlayer.Character
			local myHrp = myChar and myChar:FindFirstChild("HumanoidRootPart") :: Part?
			if not myHrp then return end
			
			local closestPlr: Player? = nil
			local shortestDist = 25 -- Khoảng cách chạm/đứng siêu gần (25 studs)
			
			for _, plr in ipairs(Players:GetPlayers()) do
				if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
					local tHrp = plr.Character.HumanoidRootPart :: Part?
					if tHrp then
			...
			if closestPlr then
				targetPlayer = closestPlr
				scanToGhimMode = false
				isGhimActive = true
				btn2.Text = "Đang Ghim: " .. closestPlr.Name
				
				if scanConnection then scanConnection:Disconnect() scanConnection = nil end
				if targetVisual then targetVisual:Destroy() end
				if trackConnection then trackConnection:Disconnect() end
				
				local targetHrp = closestPlr.Character.HumanoidRootPart :: Part?
				if targetHrp then
