...
-- Đảm bảo có PlayerGui để gắn Menu vào
local PlayerGui = LocalPlayer:WaitForChild("PlayerGui")

-- Tạo ScreenGui
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "Thudiemrena_Menu"
ScreenGui.ResetOnSpawn = false
ScreenGui.Parent = PlayerGui

-- Các bảng lưu lịch sử cho Auto Nhặt Vàng VIP (Giống hệt cơ chế bản thường)
local teleHistory = {}
local stuckItems = {}
local originalPosition = nil

-- Biến điều khiển cho tính năng Auto Vàng VIP
local autoGoldVipActive = false
local currentTarget = nil
local targetTimeStart = 0

----------------------------------------------------------------
-- CÁC HÀM BỔ TRỢ HỆ THỐNG
----------------------------------------------------------------

-- Hàm tạo chữ xuyên tường
local function createTag(target, text, color)
    if target:FindFirstChild("ESP_Tag") then return end
...
-- Kiểm tra có phải NPC (Bot) không
...
-- Định vị Item có thể nhặt được
local function getInteractableItem(obj)
    if obj:IsA("TouchTransmitter") or obj:IsA("ProximityPrompt") then
        local parent = obj.Parent
        if parent and parent:IsA("BasePart") and not parent:FindFirstAncestorOfClass("Player") and not parent:FindFirstChildOfClass("Humanoid") then
            return parent
        elseif parent and parent:IsA("Model") and not Players:GetPlayerFromCharacter(parent) and not parent:FindFirstChildOfClass("Humanoid") then
            return parent
        end
    end
    return nil
...
----------------------------------------------------------------
-- 1. TẠO GIAO DIỆN (TOGGLE BAR & MENU)
----------------------------------------------------------------
local ToggleBar = Instance.new("Frame")
ToggleBar.Name = "ToggleBar"
ToggleBar.Size = UDim2.new(0, 220, 0, 40)
ToggleBar.Position = UDim2.new(0.05, 0, 0.1, 0)
ToggleBar.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
ToggleBar.BorderSizePixel = 0
ToggleBar.Active = true
ToggleBar.Draggable = true
ToggleBar.Parent = ScreenGui

local ToggleBarCorner = Instance.new("UICorner")
ToggleBarCorner.CornerRadius = UDim.new(0, 10)
ToggleBarCorner.Parent = ToggleBar

local ToggleBarStroke = Instance.new("UIStroke")
ToggleBarStroke.Color = Color3.fromRGB(80, 80, 80)
ToggleBarStroke.Thickness = 2
ToggleBarStroke.Parent = ToggleBar
...
local MainMenu = Instance.new("Frame")
MainMenu.Name = "MainMenu"
MainMenu.Size = UDim2.new(0, 220, 0, 385)
MainMenu.Position = UDim2.new(0, 0, 1, 5)
MainMenu.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
MainMenu.BorderSizePixel = 0
MainMenu.Visible = false
MainMenu.Parent = ToggleBar
...
-- Credit meomeostaylike
local CreditsContainer = Instance.new("Frame")
CreditsContainer.Name = "CreditsContainer"
CreditsContainer.Size = UDim2.new(1, 0, 0, 24)
CreditsContainer.BackgroundTransparency = 1
CreditsContainer.Parent = MainMenu

local CreditsText1 = Instance.new("TextLabel")
CreditsText1.Size = UDim2.new(0.5, 0, 1, 0)
CreditsText1.BackgroundTransparency = 1
CreditsText1.Text = "scripts by "
CreditsText1.TextColor3 = Color3.fromRGB(150, 150, 150)
CreditsText1.TextSize = 13
CreditsText1.Font = Enum.Font.SourceSansItalic
CreditsText1.TextXAlignment = Enum.TextXAlignment.Right
CreditsText1.Parent = CreditsContainer

local CreditsText2 = Instance.new("TextLabel")
CreditsText2.Size = UDim2.new(0.5, 0, 1, 0)
CreditsText2.Position = UDim2.new(0.5, 2, 0, 0)
CreditsText2.BackgroundTransparency = 1
CreditsText2.Text = "meomeostaylike"
CreditsText2.TextColor3 = Color3.fromRGB(255, 255, 255)
CreditsText2.TextSize = 13
CreditsText2.Font = Enum.Font.SourceSansBold
...
task.spawn(function()
    local hue = 0
    while true do
        CreditsText2.TextColor3 = Color3.fromHSV(hue, 1, 1)
        hue = hue + 0.01
...
Padding.Parent = MainMenu

-- Hàm tạo các nút bật/tắt tính năng
local function createFeatureToggle(name, defaultState, callback)
...
MenuToggleBtn.MouseButton1Click:Connect(function()
    ...
    if menuActive then
        MenuToggleBtn.Text = "On: ●"
        MenuToggleBtn.BackgroundColor3 = Color3.fromRGB(46, 204, 113)
...
----------------------------------------------------------------
-- 2. KHAI BÁO CÁC CHỨC NĂNG PHỤ
----------------------------------------------------------------

-- [CHỨC NĂNG 1]: ESP NPC
local npcEspActive = false
local function applyNPCEsp(desc)
    if npcEspActive and isNPC(desc) then
        if not desc:FindFirstChild("NPC_ESP") then
            ...
            createTag(desc, "Bot", Color3.fromRGB(255, 0, 0))
        end
    end
end

createFeatureToggle("Esp NPC", false, function(enabled)
    npcEspActive = enabled
    if enabled then
        for _, desc in ipairs(Workspace:GetDescendants()) do applyNPCEsp(desc) end
    else
        for _, desc in ipairs(Workspace:GetDescendants()) do
            local esp = desc:FindFirstChild("NPC_ESP") if esp then esp:Destroy() end
            local tag = desc:FindFirstChild("ESP_Tag") if tag then tag:Destroy() end
        end
    end
end)

-- [CHỨC NĂNG 2]: AUTO BÁN ITEM
local autoSellActive = false
local targetSellPrompt = nil
local promptTriggerConn = nil
...
local function startAutoSellLoop()
    while autoSellActive do
        ...
        if targetSellPrompt and targetSellPrompt.Parent and char and hum and hum.Health > 0 then
            local itemToSell = nil
            if backpack then
                for _, child in ipairs(backpack:GetChildren()) do
                    if child:IsA("Tool") and child.Name ~= "Tele Tool" then
                        itemToSell = child
                        break
                    end
                end
            ...
            if itemToSell then
                if itemToSell.Parent == backpack then
                    hum:EquipTool(itemToSell)
                end
                fireproximityprompt(targetSellPrompt, 1)
            end
        end
    end
end

createFeatureToggle("Auto Bán", false, function(enabled)
    autoSellActive = enabled
    if enabled then
        promptTriggerConn = ProximityPromptService.PromptTriggered:Connect(function(prompt, player)
            if player == LocalPlayer then
                targetSellPrompt = prompt
                local parentPart = prompt.Parent
                if parentPart and parentPart:IsA("BasePart") then
                    local existingTag = parentPart:FindFirstChild("Sell_Target_Tag")
                    if not existingTag then
                        local tag = Instance.new("Folder")
                        tag.Name = "Sell_Target_Tag"
                        tag.Parent = parentPart
                        createTag(parentPart, "MÁY BÁN ĐÃ KHÓA!", Color3.fromRGB(46, 204, 113))
                    end
                end
            end
        end)
        ...
        if promptTriggerConn then
            promptTriggerConn:Disconnect()
            promptTriggerConn = nil
        end
        if targetSellPrompt and targetSellPrompt.Parent then
            local parentPart = targetSellPrompt.Parent
            local tagFolder = parentPart:FindFirstChild("Sell_Target_Tag")
            if tagFolder then tagFolder:Destroy() end
            local tagESP = parentPart:FindFirstChild("ESP_Tag")
            if tagESP then tagESP:Destroy() end
        end
        targetSellPrompt = nil
    end
end)

-- [CHỨC NĂNG 3]: SHOW MÁU
local HealthGui = Instance.new("Frame")
HealthGui.Size = UDim2.new(0, 20, 0, 200)
HealthGui.Position = UDim2.new(1, -40, 0.5, -100)
HealthGui.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
HealthGui.Visible = false
HealthGui.Parent = ScreenGui

local HealthBar = Instance.new("Frame", HealthGui)
HealthBar.Size = UDim2.new(1, 0, 1, 0)
HealthBar.BackgroundColor3 = Color3.fromRGB(46, 204, 113)

local HealthText = Instance.new("TextLabel", HealthGui)
HealthText.Position = UDim2.new(-5, 0, 0, 0)
HealthText.Size = UDim2.new(0, 100, 0, 30)
HealthText.BackgroundTransparency = 1
HealthText.Text = "『100%』"
HealthText.TextColor3 = Color3.fromRGB(255, 255, 255)

createFeatureToggle("Show máu", false, function(enabled)
    HealthGui.Visible = enabled
    if enabled then
        RunService.RenderStepped:Connect(function()
            if not HealthGui.Visible then return end
            local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
            if hum then
                local pct = math.clamp(hum.Health / hum.MaxHealth, 0, 1)
                HealthBar.Size = UDim2.new(1, 0, pct, 0)
                HealthText.Text = "『" .. math.floor(pct * 100) .. "%』"
            end
        end)
    end
end)

-- [CHỨC NĂNG 4]: ESP ITEM
local itemEspActive = false

local function applyItemESP(obj)
    local target = getInteractableItem(obj)
    if itemEspActive and target then
        if not target:FindFirstChild("Item_ESP") then
            local highlight = Instance.new("Highlight")
            ...
            createTag(target, "Item", Color3.fromRGB(255, 255, 255))
        end
    end
end

createFeatureToggle("Esp Item", false, function(enabled)
    itemEspActive = enabled
    if enabled then
        for _, desc in ipairs(Workspace:GetDescendants()) do applyItemESP(desc) end
    else
        for _, desc in ipairs(Workspace:GetDescendants()) do
            local esp = desc:FindFirstChild("Item_ESP") if esp then esp:Destroy() end
            local tag = desc:FindFirstChild("ESP_Tag") if tag then tag:Destroy() end
        end
    end
end)

-- [CHỨC NĂNG 5]: GODMODE BOT NÂNG CẤP
local godmodeActive = false
local originalData = {}

local function disableBotHitbox(bot)
    if isNPC(bot) then
        for _, part in ipairs(bot:GetDescendants()) do
            if part:IsA("BasePart") then
                ...
                if part.Name:lower():find("arm") or part.Name:lower():find("leg") or part.Name:lower():find("hand") or part.Name:lower():find("foot") or part:FindFirstChildOfClass("TouchTransmitter") then
                    part.CFrame = part.CFrame * CFrame.new(0, -500, 0)
                end
            end
        end
    end
...
local function restoreBotHitbox(bot)
    if isNPC(bot) then
        for _, part in ipairs(bot:GetDescendants()) do
            if part:IsA("BasePart") and originalData[part] then
                part.CanTouch = originalData[part].CanTouch
                part.CanCollide = originalData[part].CanCollide
                part.CFrame = originalData[part].CFrame
            end
        end
    end
end

createFeatureToggle("Godmode Bot", false, function(enabled)
    godmodeActive = enabled
    if enabled then
        task.spawn(function()
            while godmodeActive do
                for _, desc in ipairs(Workspace:GetDescendants()) do
                    disableBotHitbox(desc)
                end
                task.wait(0.5)
            end
        end)
        ...
        for _, desc in ipairs(Workspace:GetDescendants()) do
            restoreBotHitbox(desc)
        end
        originalData = {}
    end
end)

-- [CHỨC NĂNG 6]: TELEPORT TOOL
local teleToolActive = false
local currentTool = nil

local function clearTeleTool()
    local toolChar = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Tele Tool")
    if toolChar then toolChar:Destroy() end
    
    local backpack = LocalPlayer:FindFirstChild("Backpack")
    local toolBack = backpack and backpack:FindFirstChild("Tele Tool")
    if toolBack then toolBack:Destroy() end
    
    currentTool = nil
end

createFeatureToggle("Teleport Tool", false, function(enabled)
    teleToolActive = enabled
    if enabled then
...
-- [CHỨC NĂNG 7]: AUTO NHẶT THƯỜNG (Giữ nguyên cho bạn dùng song song)
local autoPickActive = false
task.spawn(function()
    while task.wait(0.05) do
        if autoPickActive then
            local char = LocalPlayer.Character
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if root then
                ...
                for _, desc in ipairs(Workspace:GetDescendants()) do
                    local itemObj = getInteractableItem(desc)
                    if itemObj and not stuckItems[itemObj] then
                        local teleCount = teleHistory[itemObj] or 0
                        if teleCount < 2 then
                            local itemPos = itemObj:IsA("BasePart") and itemObj.Position or itemObj:GetPivot().Position
                ...
                if closestItem then
                    if currentTarget ~= closestItem then
                        currentTarget = closestItem
                        targetTimeStart = os.clock()
                    else
                        if os.clock() - targetTimeStart > 1.5 then
                            stuckItems[closestItem] = true
                            currentTarget = nil
                            continue
                        end
                    end
                    
                    teleHistory[closestItem] = (teleHistory[closestItem] or 0) + 1
                    
                    if closestItem:IsA("BasePart") then
                        root.CFrame = closestItem.CFrame
                    else
                        root.CFrame = closestItem:GetPivot()
                    end
                    
                    local prompt = closestItem:FindFirstChildOfClass("ProximityPrompt") or closestItem.Parent:FindFirstChildOfClass("ProximityPrompt")
                    if prompt then
...
createFeatureToggle("Auto Nhặt", false, function(enabled)
    autoPickActive = enabled
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    
    if enabled then
        if root then originalPosition = root.CFrame end
    else
        teleHistory = {}
        stuckItems = {}
        currentTarget = nil
...
----------------------------------------------------------------
-- 4. [CHỨC NĂNG 8 - CẬP NHẬT VIP]: AUTO VÀNG VIP "CHẠM & BIẾN" 
----------------------------------------------------------------
local currentGoldTarget = nil
local goldTargetTimeStart = 0
local goldTeleHistory = {}
local goldStuckItems = {}
local goldOriginalPosition = nil

task.spawn(function()
    while task.wait(0.05) do
        if autoGoldVipActive then
            local char = LocalPlayer.Character
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if root then
                local closestGold = nil
                local shortestDistance = math.huge -- Quét không giới hạn khoảng cách trên toàn map
                
                -- Tìm kiếm thông minh các item thoả mãn điều kiện
                for _, desc in ipairs(Workspace:GetDescendants()) do
                    local nameLower = desc.Name:lower()
                    local hasPromptText = false
                    
                    -- Danh sách các từ khóa cần tìm
                    local targetKeywords = {"vàng", "gold", "bond", "đồng", "phế liệu", "kim cương", "tiền", "đèn pin"}
                    
                    local prompt = desc:FindFirstChildOfClass("ProximityPrompt")
                    if prompt then
                        local pText = (prompt.ObjectText or "") .. " " .. (prompt.ActionText or "")
                        for _, kw in pairs(targetKeywords) do
                            if pText:lower():find(kw) then hasPromptText = true break end
                        end
                    end
                    ...
                    local isMatch = false
                    for _, kw in pairs(targetKeywords) do
                        if nameLower:find(kw) then isMatch = true break end
                    end
                    
                    if isMatch or hasPromptText then
                        local itemObj = getInteractableItem(desc) or desc
                        if itemObj and not goldStuckItems[itemObj] then
                            local teleCount = goldTeleHistory[itemObj] or 0
                            if teleCount < 2 then 
                                local itemPos = itemObj:IsA("BasePart") and itemObj.Position or itemObj:GetPivot().Position
                                local dist = (root.Position - itemPos).Magnitude
                                if dist < shortestDistance then
                ...
                if closestGold then
                    if currentGoldTarget ~= closestGold then
                        currentGoldTarget = closestGold
                        goldTargetTimeStart = os.clock()
                    else
                        if os.clock() - goldTargetTimeStart > 1.5 then
                            goldStuckItems[closestGold] = true
                            currentGoldTarget = nil
                            continue
                        end
                    end
                    
                    goldTeleHistory[closestGold] = (goldTeleHistory[closestGold] or 0) + 1
                    root.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
                    
                    if closestGold:IsA("BasePart") then
                        root.CFrame = closestGold.CFrame
                    else
                        root.CFrame = closestGold:GetPivot()
                    end
                    
                    local promptToFire = closestGold:FindFirstChildOfClass("ProximityPrompt") or closestGold.Parent:FindFirstChildOfClass("ProximityPrompt")
                    if promptToFire then
                        fireproximityprompt(promptToFire, 1)
                    end
...
createFeatureToggle("Auto Vàng VIP", false, function(enabled)
    autoGoldVipActive = enabled
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    
    if enabled then
        if root then goldOriginalPosition = root.CFrame end
    else
        goldTeleHistory = {}
        goldStuckItems = {}
        currentGoldTarget = nil
        if goldOriginalPosition and root then
            task.wait(0.1)
            root.CFrame = goldOriginalPosition
            goldOriginalPosition = nil
        end
    end
end)

----------------------------------------------------------------
-- LẮNG NGHE EVENT THỰC THỂ MỚI SPAWN
----------------------------------------------------------------
